...
var TransactionError = function(code, data) {
  Error.captureStackTrace(this, this.constructor);
  this.message = errcode[code];

  this.code = code;
  this.data = data;
};

require('util').inherits(TransactionError, Error);
...
Transaction.statics.errors = function() {
  return Object.keys(errcode);
}

Transaction.statics.getError = function(code) {
  return errcode[code];
}
  ...
  tx.save(function(err){
    if(err && err.message && err.message.indexOf('E11000 ') !== -1) {  // Duplicate key
      ...
      if(!giveUp){
        ...
        callback(new TransactionError('E_TIMEOUT'));
      }
    }else if(err){
      throw err;
    }else{
  ...
  self.fetchPreviousVersions(function(err, tx){
    ...
    Transaction.findOneAndUpdate(
      ...
      function(err,doc){
        if(err){ throw err; }
        if(!doc){ 
          callback(new TransactionError('E_CANNOT_START', self._id));
        }else{
...
Transaction.methods.fetchPreviousVersion = function(tx, change, callback){
  ...
  actions.findOne(change, function(err, cdoc){
    if(err) { 
      throw new TransactionError('E_FETCH_PREV');
    }else{
      prev = cdoc;
    }
    Transaction.findOneAndUpdate(
      ...
      function(err, doc){
        if(err){ throw err; }
        if(!doc){
          callback(new TransactionError('E_PREV_SAVE', self._id));
        }else{
          callback();
        }
      }
  ...
  //had to pass 'self' in manually
    Transaction.findOneAndUpdate(
      ...
      function(err, doc){
        if(err){ throw err; }
        if(!doc){
          callback(new TransactionError('E_CHANGE_START', self._id));
        }else{
...
Transaction.methods.handleChangeResult = function(tx, change, msg, state, cb){
  ...
  Transaction.findOneAndUpdate(
    ...
    function(err, doc){
      if(err){ throw err; }
      if(!doc){
        cb(new TransactionError('E_CHANGE_UPDATE', self._id));
      }else{
        if(state === 'error'){
...
Transaction.methods.finish = function(callback){
  ...
  Transaction.findOneAndRemove(
    ...
    function(err, doc){
      if(err){ throw err; }
      if(!doc){
        callback(new TransactionError('E_FINISH', self._id));
      }else{
        callback();
      }
    }
...
Transaction.methods.revert = function(err, callback){
  ...
  Transaction.findOneAndUpdate(
    ...
    function(err,doc){
      if(err){ throw err; }
      if(!doc){ 
        callback(new TransactionError('E_REVERT', self._id));
      }else{
...
Transaction.statics.cleanupOld = function(tx, callback){
  Transaction.findOneAndUpdate(
    ...
    function(err,doc){
      if(err){ throw err; }
      if(!doc){ 
        callback(new TransactionError('E_CLEANUP', tx._id));
      }else{
        doc.revertChanges(callback);
      }
    }
  ...
  if(change.state === 'new' || change.state === 'error'){
    Transaction.findOneAndUpdate(
      ...
      function(err, doc){
        if(err){ throw err; }
        if(!doc){
          callback(new TransactionError('E_CHANGE_CANCEL', self._id));
        }else{
          callback(err, doc);
        }
      }
...
Transaction.methods.handleRevertResult = function(tx, change, msg, state, cb){
  ...
  Transaction.findOneAndUpdate(
    ...
    function(err, doc){
      ...
      if(err){ throw err; }
      if(!doc){
        cb(new TransactionError('E_FAILED', self._id));
      }else{
        if(state === 'cancelerror'){
          cb(new TransactionError('E_CHANGE_REVERT', change.coll +': '+ change.docId));
        }else{
          cb();
        }
      }
  ...
  Transaction.findOneAndUpdate(
    ...
    function(err, doc){
      if(err){
        ...
        throw new TransactionError('E_MARK_ERROR', self._id);
      }else{
...
Transaction.methods.completeRevert = function(callback){
  ...
  this.remove(function(err) {
    if (err) callback(new TransactionError('E_ROLLBACK_REMOVE'));
    callback(new TransactionError('E_ROLLBACK_SUCCESS'));
  });
};
